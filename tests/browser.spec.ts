...
test('desktop signup, verification, onboarding, image upload, team, and access settings', async ({
  ...
  await expect(page.getByRole('button', { name: 'Go to workspace', exact: true })).toBeDisabled()
  ...
  await page.getByLabel('Company description').fill('Updated company description')
  await page.getByRole('button', { name: 'Continue', exact: true }).click()
  await expect(page.getByLabel('First name')).toHaveValue('Alex')
  await expect(page.getByLabel('Job title')).toHaveValue('Founder & Designer')
  await expect(page).toHaveURL(profileUrl)
  await page.getByRole('button', { name: 'Back', exact: true }).click()
  await expect(page.getByLabel('Company description')).toHaveValue('Updated company description')
  await page.getByRole('button', { name: 'Continue', exact: true }).click()
