...
test('Figma desktop dimensions, field states, onboarding cards, and avatar selection', async ({
  ...
  const avatarPreview = page.locator('.profile-preview .avatar img')
  const gradientSource = await avatarPreview.getAttribute('src')
  await page.getByLabel('First name').fill('Barly')
  await page.getByLabel('Last name').fill('Repo')
  await expect(avatarPreview).toHaveAttribute('src', gradientSource!)
  await page.getByRole('button', { name: 'Choose avatar', exact: true }).click()
  await page.getByRole('button', { name: 'Choose avatar 1', exact: true }).click()
  await expect(page.getByRole('dialog')).not.toBeVisible()
  await expect(page.locator('.profile-preview img[src^="data:image/png"]')).toBeVisible()
  const brSource = await avatarPreview.getAttribute('src')
  await page.getByLabel('First name').fill('')
  await page.getByLabel('Last name').fill('')
  await expect(avatarPreview).not.toHaveAttribute('src', brSource!)
  const emptySource = await avatarPreview.getAttribute('src')
  await page.getByLabel('First name').fill('Barly')
  await expect(avatarPreview).not.toHaveAttribute('src', emptySource!)
  await page.getByLabel('Last name').fill('Repo')
  await expect(avatarPreview).toHaveAttribute('src', brSource!)
  await expect(page.locator('.image-picker .avatar img')).toHaveAttribute('src', brSource!)
  await page.getByLabel('First name').fill('Alex')
  await page.getByLabel('Last name').fill('Carter')
  await page.getByRole('heading').first().click()
